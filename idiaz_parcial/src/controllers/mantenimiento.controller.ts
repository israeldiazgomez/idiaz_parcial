import { Request, Response } from "express";
import {MantenimientoI, Mantenimiento} from '../models/Mantenimiento';

export class MantenimientoController{
    public async getMantenimientos(req: Request, res: Response){
        try {
            const mantenimientos = await Mantenimiento.findAll()
            if (!mantenimientos){
                res.status(400).json({msg: 'EQUIPO MALO'})

            }
            return res.status(200).json({mantenimientos})
        } catch (error) {
            res.status(500).json({msg: 'ERROR DETECTADO POR FAVOR REGRESE'})
            
        }
    }
    public async createMantenimiento(req: Request, res: Response){
        const body: MantenimientoI = req.body;
        try {
            if((!body.fecha && !body.descripcion && !body.resultado)) return res.status(400).json({msg: 'some data is requeried'})
            const mantenimientoExist: Mantenimiento | null = await Mantenimiento.findOne(
                {
                    where: {fecha: body.fecha},
                }
            )
            if(mantenimientoExist){
                return res.status(400).json({msg: "fecha ya existe ingrese otro "})
            }
            const mantenimiento = await Mantenimiento.create(body);
            res.status(200).json({mantenimiento})
        } catch (error) {
            res.status(500).json({msg: "ERROR AL CREAR"})
        }
    }
    public async borrarMantenimiento(req: Request, res: Response){
        try {
            const {id} = req.body;
            const response = await Mantenimiento.destroy({
                where: {id: id}
            })
            .then(function(data){
                const res = {success: true, data: data, message: "ejecudado el recorrido"}
                return res;
            })
            .catch(error => {
                const res = { success: false, error: error}
                return res;
            })
            res.json(response);

        } catch (e) {
            console.log(e);
            
        }
    }
}